...
<div class="easyui-layout" fit="true" style="margin: 0px;border: 0px;overflow: hidden;width:100%;height:100%;">
    <div data-options="region:'north',collapsed:false,split:false,border:false"
        ...
        <div class="search">
            <form id="actRoot_search_form">
                <span class="eu-label">活动名称:</span>
                <input type="text" name="sc_actRootName" class="easyui-validatebox textbox eu-input"
                ...
                <a class="easyui-linkbutton" href="javascript:void(0);"
                   ...
                   onclick="javascript:$actRoot_search_form.form('reset');">重置查询</a>
                &nbsp;
                <a class="easyui-linkbutton" href="javascript:void(0);" data-options="plain:true,iconCls:'eu-icon-add'"
                   onclick="javascript:showDialog();">添加活动</a>
            </form>
        </div>
    </div>
    <%-- 中间部分 列表 --%>
    <div data-options="region:'center',split:false,border:false"
        ...
        <table id="actRoot_datagrid"></table>
    </div>
</div>


<script>
    var $actRoot_datagrid;
    var $actRoot_search_form;
    $(function () {
        $actRoot_search_form = $('#actRoot_search_form').form();
        //数据列表
        $actRoot_datagrid = $('#actRoot_datagrid').datagrid({
            url: ctxAdmin + '/actRoot/actRootDatagrid',
            fit: true,
            ...
            frozenColumns: [
                [
                    ...
                    {field: 'actCount', title: '子活动数量', width: 80},
     ...
     */
    function search() {
        $actRoot_datagrid.datagrid('load', $.serializeObject($actRoot_search_form));
    }

    // 删除用户
    function del(indexRow) {
        var row = getRow(indexRow);
        $.messager.confirm('确认提示！', '您确定要删除此活动?', function (r) {
            if (r) {
                $.ajax({
                    url: ctxAdmin + '/actRoot/delete/'+row.id,
                    type: 'post',
                    traditional: true,
                    dataType: 'json',
                    success: function (data) {
                        if (data.code == 1) {
                            $actRoot_datagrid.datagrid('load');	// reload the act data
    ...
    function formInit() {
        $actRoot_form = $('#actRoot_form').form({
            url: ctxAdmin + '/actRoot/bsave',
            ...
            success: function (data) {
                ...
                if (json.code == 1) {
                    ...
                    $actRoot_datagrid.datagrid('reload');//重新加载列表数据
    ...
    function getRow(rowIndex) {
        var rows = $actRoot_datagrid.datagrid('getRows');
        return rows[rowIndex];
    }

    function showDialog(rowIndex) {
        var row;
        var inputUrl = ctxAdmin + "/actRoot/input";
