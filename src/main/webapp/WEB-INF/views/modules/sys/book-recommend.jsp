...
<script type="text/javascript">
    ...
    function loadPublish(){
        $('#publishType').combobox({
            url: '${ctxAdmin}/bookRecommend/publishTypeCombobox?selectType=all',
            width: 80,
            editable:false,
            value:''
        });
...
<div class="easyui-layout" fit="true" style="margin: 0px;border: 0px;overflow: hidden;width:100%;height:100%;">
    <div data-options="region:'north',collapsed:false,split:false,border:false"
        ...
        <div class="search">
            <form id="book_recommend_search_form">
                <span class="eu-label">期刊名称:</span>
                <input type="text" name="bookRecommendName" class="easyui-validatebox textbox eu-input"
                       placeholder="请输入期刊名称..." onkeydown="if(event.keyCode==13)search()" maxLength="25"
                       style="width: 160px"/>
                &nbsp;<span class="eu-label">类型:</span><input id="publishType" name="publishType" style="height: 28px;" />
                ...
                <a class="easyui-linkbutton" href="javascript:void(0);"
                   ...
                   onclick="javascript:$book_recommend_search_form.form('reset');">重置查询</a>
                &nbsp;
    ...
    <div data-options="region:'center',split:false,border:false"
        ...
        <table id="book_recommend_datagrid"></table>
    </div>
</div>
<script>
    var $book_recommend_datagrid;
    var $book_recommend_search_form;
    $(function () {
        $book_recommend_search_form = $('#book_recommend_search_form').form();
        //数据列表
        $book_recommend_datagrid = $('#book_recommend_datagrid').datagrid({
            url: ctxAdmin + '/bookRecommend/bookRecommendDatagrid',
            fit: true,
    ...
    function search() {
        $book_recommend_datagrid.datagrid('load', $.serializeObject($book_recommend_search_form));
    }
    ...
    function del(indexRow) {
        ...
        $.messager.confirm('确认提示！', '您确定要删除此期刊?', function (r) {
            if (r) {
                $.ajax({
                    url: ctxAdmin + '/bookRecommend/delete/'+row.id,
                    type: 'post',
                    traditional: true,
                    dataType: 'json',
                    success: function (data) {
                        if (data.code == 1) {
                            $book_recommend_datagrid.datagrid('load');	// reload the book_recommend data
    ...
    function publish(indexRow) {
        ...
        $.messager.confirm('确认提示！', '您确定要发布此期刊?', function (r) {
            if (r) {
                $.ajax({
                    url: ctxAdmin + '/bookRecommend/publish/'+row.id,
                    type: 'post',
                    traditional: true,
                    dataType: 'json',
                    success: function (data) {
    ...
    function formInit() {
        $book_recommend_form = $('#book_recommend_form').form({
            url: ctxAdmin + '/bookRecommend/save',
            ...
            success: function (data) {
                ...
                if (json.code == 1) {
                    $book_recommend_dialog.dialog('destroy');//销毁对话框
                    $book_recommend_datagrid.datagrid('reload');//重新加载列表数据
    ...
    function getRow(rowIndex) {
        var rows = $book_recommend_datagrid.datagrid('getRows');
        return rows[rowIndex];
    }

    function showDialog(rowIndex) {
        var row;
        var inputUrl = ctxAdmin + "/bookRecommend/input";
        ...
        //弹出对话窗口
        $book_recommend_dialog = $('<div/>').dialog({
            title: '操作',
            top: 20,
            width: 960,
            height: 420,
            ...
            buttons: [
                ...
                },
                {
                    ...
                    handler: function () {
                        $book_recommend_dialog.dialog('destroy');
                    }
                }
            ],
            onClose: function () {
            ...
            onLoad: function () {
                ...
                if (row) {
                    ...
                    $book_recommend_form.form('load', row);
                    editor.ready(function() {
                        editor.setContent(row.periodInfo);
                    });
                }else{
                    $("input[name='periodType'][value='1']").attr("checked", 'checked');//状态 初始化值
                }
